...
enum {
	WINDOW_WIDTH = 433, // 1024
	WINDOW_HEIGHT = 642 // 768
};
...
void TestAppDelegate::preinit()
{
	std::string appDataFolder = getAppDataPath();
	Core::fileSystem.SetWriteDirectory(appDataFolder);
}
...
void TestAppDelegate::RegisterTypes()
{
	...
	//
	REGISTER_WIDGET_XML(MoveTheBoxGUIWidget, "MoveTheBoxGUIWidget");
}
...
void TestAppDelegate::OnPostDraw() {
	if (!Render::isFontLoaded(s_programmMessageFont))
		return;
